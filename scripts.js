...
document.addEventListener('DOMContentLoaded', () => {
  // Custom‐text UI
  const customSelect = document.getElementById('customTextSelect');
  const customInput  = document.getElementById('customTextInput');
  const applyBtn     = document.getElementById('applyTextButton');
  customInput.style.display = 'none';
  applyBtn   .style.display = 'none';

  // Random preset at load
  (()=>{
    const opts = Array.from(customSelect.options).filter(o => o.value !== 'custom');
    customSelect.value = opts[Math.floor(Math.random()*opts.length)].value;
    updateShareImage();
  })();

  customSelect.addEventListener('change', () => {
    const isCustom = customSelect.value === 'custom';
    customInput.style.display = isCustom ? 'block' : 'none';
    applyBtn   .style.display = isCustom ? 'inline-block' : 'none';
    if (!isCustom) updateShareImage();
  ...
  document.getElementById('submitReviewForm')?.addEventListener('click', () => {
    ...
    document.getElementById('reviewShareImage').src =
      `${REVIEW_TEMPLATE_URL}first_name=&job_title=${encodeURIComponent(val)}`;
